...
| Google Flights check says "Invalid API key" | the "API credentials" proxy header (Bearer) is rejected by SerpApi | fixed 26 Sep: key set as the `SERPAPI_KEY` environment variable in the cloud environment, sent as `api_key`; `serp_check.py` uses it first |
