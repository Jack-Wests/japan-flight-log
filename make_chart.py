#!/usr/bin/env python3
"""Regenerate chart.png from the full prices.csv price log.

Best-total-per-person line over time, with a green BUY zone (<=$800),
amber WATCH band ($800-$950), and a marker on today's (latest) point.
"""
import csv
...
ax.text(0.012, BUY - 6, "  BUY zone  ≤ $800", transform=ax.get_yaxis_transform(),
        va="top", ha="left", fontsize=9, color="#1b5e20", fontweight="bold")
ax.text(0.012, WATCH - 6, "  WATCH  $800–$950", transform=ax.get_yaxis_transform(),
...
print("wrote chart.png with", len(dates), "point(s)")
